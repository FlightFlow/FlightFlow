...
class Requester {
  ...
  // TODO: add a token rotation handler

  async sendRequest<
    TResponseData = null,
    TRequestPayload = UtilTypes.RequesterBaseTypes.RequesterConfigParams,
  >(): Promise<GlobalTypes.ServerResponseParams<TResponseData>> {
    ...
    >(axiosConfig);
    return response.data as GlobalTypes.ServerResponseParams<TResponseData>;
  }
}
